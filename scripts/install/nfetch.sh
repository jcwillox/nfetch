...
usage() {
	this="$1"
	cat <<EOF
${this}: download nfetch and optionally run nfetch

Usage: ${this} [-b bindir] [-d] [-t tag] [nfetch-args]
  -b sets the installation directory, default is ${BINDIR}.
  -d enables debug logging.
  -t sets the tag, default is ${TAG}.
If nfetch-args are given, after install nfetch is executed with nfetch-args.
EOF
	exit 2
}
...
main() {
	...
	# download tarball
	NAME="nfetch_${VERSION}_${GOOS}_${GOARCH}"
	TARBALL="${NAME}.${FORMAT}"
