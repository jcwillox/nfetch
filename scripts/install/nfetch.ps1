...
function get-real-tag {
    ...
    $release_url = "https://github.com/jcwillox/nfetch/releases/$tag"
...
<#
    .SYNOPSIS
    Install 'nfetch' the system information utility

    .DESCRIPTION
    Installs nfetch to the given directory, defaulting to ~/.local/bin

    You can specify a particular git tag using the -Tag option.

    Examples:
    '$params = "-BinDir ~/bindir"', (iwr https://tinyurl.com/nfetch-ps1).Content | powershell -c -
    '$params = "-Tag v1.8.10"', (iwr https://tinyurl.com/nfetch-ps1).Content | powershell -c -
#>
function Install-Nfetch {
    [CmdletBinding(PositionalBinding=$false)]
    param(
    ...
    $github_download = "https://github.com/jcwillox/nfetch/releases/download"
    New-Item -Type Directory -Path $tempdir | Out-Null

    # download tarball
    $name="nfetch_$($version)_$($os)_$($arch)"
    $tarball="$name.$format"
...
try {
    Invoke-Expression ("Install-Nfetch " + $params)
} catch {
